...
public class GuiConfig {
    public static Cache load(JImGui imGui){
        Cache loadOutput = new Cache();
        ...
        for(int i = 0; i < 6; i++){
            JImFont font = fonts.addFontFromFile("res/hacky.ttf", 12 + (6*i));
            if(font.isLoaded()){
        ...
        loadOutput.setHacky16(fonts.addFontFromFile("res/hacky.ttf", 16));

        return loadOutput;
    }

    public static void render(JImGui imGui, Cache loadOutput){
        // * gen. design
